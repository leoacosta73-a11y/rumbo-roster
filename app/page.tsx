...
function DayDetail({duty,onClose}:{duty:Duty,onClose:()=>void}){
  ...
  return <div className="scrim" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><section className="detail" role="dialog" aria-modal="true" aria-label={`Detalle del ${day.getDate()} de ${months[day.getMonth()]}`}>
    ...
    <div className="time-head"><span>HORA LOCAL</span><span>ZULU</span></div>
    <div className="timeline">
      <div className="point"><i/><div><small>PRESENTACIÓN {duty.reportPlace&&`· ${duty.reportPlace}`}</small><strong>{duty.report||"—"}</strong></div><b>{duty.reportZ||"—"} Z</b></div>
      {duty.legs?.map((leg,i)=><div className="point leg" key={leg.flight}><i/><div><small>TRAMO {i+1} · {leg.flight}</small><strong>{leg.from} <em>→</em> {leg.to}</strong><p>{leg.departL} — {leg.arriveL} · Bloque {leg.block}</p></div><b>{leg.departZ} — {leg.arriveZ} Z</b></div>)}
      <div className="point end"><i/><div><small>FINALIZACIÓN</small><strong>{duty.end||"—"}</strong></div><b>{duty.endZ||"—"} Z</b></div>
    </div>
...
export default function Home(){
  const [view,setView]=useState<"month"|"agenda">("month"); const [selected,setSelected]=useState<Duty|null>(null); const [showImport,setShowImport]=useState(false); const [source,setSource]=useState("Demostración ARMS"); const [filter,setFilter]=useState("all");
  const y=2025,m=6; const first=(new Date(y,m,1).getDay()+6)%7; const days=new Date(y,m+1,0).getDate();
  const cells=Array.from({length:42},(_,i)=>{const d=i-first+1; if(d<1){const prev=new Date(y,m,d);return {n:prev.getDate(),date:key(prev.getFullYear(),prev.getMonth(),prev.getDate()),muted:true}} if(d>days){return {n:d-days,date:key(y,m+1,d-days),muted:true}} return {n:d,date:key(y,m,d),muted:false}});
  ...
  return <main>
    <header><a className="brand" href="#"><span>R</span><div><b>Rumbo</b><small>MI ROSTER</small></div></a><nav><button className={view==="month"?"active":""} onClick={()=>setView("month")}>Calendario</button><button className={view==="agenda"?"active":""} onClick={()=>setView("agenda")}>Agenda</button></nav><button className="import" onClick={()=>setShowImport(true)}>⇧ <span>Importar roster</span></button></header>
    <div className="shell">
      ...
      <section className="next-card" onClick={()=>setSelected(next)}><div className="date-box"><small>SÁB</small><strong>19</strong><span>JUL</span></div><div className="next-main"><p>PRÓXIMA ACTIVIDAD · EN 3 DÍAS</p><h2><span className="plane">✈</span> EZE <em>→</em> NQN <i/> NQN <em>→</em> AEP <i/> +2</h2><div><span>◷ Presentación <b>06:00</b></span><span>⌖ Ezeiza</span><span>◴ 7 h 16 min de vuelo</span></div></div><button aria-label="Abrir detalle">›</button></section>
      ...
      {view==="month"?<section className="calendar"><div className="weekdays">{weekdays.map(w=><span key={w}>{w}</span>)}</div><div className="grid">{cells.map(c=>{const d=byDate.get(c.date);return <button key={c.date} className={`day ${c.muted?"muted":""} ${d?`has ${d.type}`:""}`} onClick={()=>d&&setSelected(d)}><span className="num">{c.n}</span>{d&&<div className="activity"><span className="activity-icon">{icon[d.type]}</span><b>{d.type==="flight"?`${d.legs?.length} vuelos`:d.title}</b>{d.type==="flight"&&<small>{d.report} · {d.reportPlace}</small>}</div>}</button>})}</div></section>:
      <section className="agenda"><div className="agenda-head"><span>Fecha</span><span>Actividad</span><span>Presentación</span><span>Final</span></div>{agenda.map(d=>{const dt=new Date(d.date+"T12:00");return <button key={d.date} onClick={()=>setSelected(d)}><div className="agenda-date"><strong>{dt.getDate()}</strong><span>{weekdays[(dt.getDay()+6)%7]}</span></div><span className={`agenda-icon ${d.type}`}>{icon[d.type]}</span><div><b>{d.type==="flight"?d.legs?.map(l=>`${l.from}–${l.to}`).join(" · "):d.title}</b><small>{d.note||`${d.legs?.length||0} tramos · ${d.total}`}</small></div><time>{d.report} <small>{d.reportPlace}</small></time><time>{d.end}</time><i>›</i></button>})}</section>}
    ...
    {selected&&<DayDetail duty={selected} onClose={()=>setSelected(null)}/>} {showImport&&<ImportModal onClose={()=>setShowImport(false)} onImported={setSource}/>}<div className="offline-badge">✓ Funciona offline</div>
  </main>
}
