...
async function parseRosterPdf(buf:ArrayBuffer){
  ...
  matches.forEach((match,i)=>{const date=`${match[3]}-${pad(pdfMonths[match[2]]+1)}-${pad(Number(match[1]))}`;if(seen.has(date))return;seen.add(date);const start=Math.max(0,(match.index||0)-550),end=Math.min(text.length,(matches[i+1]?.index||match.index!+800)+150);const window=text.slice(start,end);let type:Duty["type"]="flight",title="Jornada de vuelo";if(/R-OFF|\bOFF\b/i.test(window)){type="off";title=/R-OFF/i.test(window)?"R-OFF":"OFF"}else if(/\bNDA\b/i.test(window)){type="nda";title="NDA"}else if(/STBY|STANDBY|GUARDIA/i.test(window)){type="standby";title="STBY"}else if(/\bSIM\b/i.test(window)){type="sim";title="Simulador"}else if(/GROUND|TRAINING|CAPACIT/i.test(window)){type="training";title="Capacitación"}else if(/LEAVE|VAC\d*/i.test(window)){type="leave";title="Licencia"}
    ...
    parsed.push({date,type,title,report:local[0],reportZ:zulu[0],reportPlace:places[0]?.[1],end:local.at(-1),endZ:zulu.at(-1),note:`Importado desde ARMS${flights.length?` · ${flights.join(" · ")}`:""}`})});
...
function ImportModal({onClose,onImported}:{onClose:()=>void,onImported:(name:string,items:Duty[])=>void}){
  ...
  const read=async(file?:File)=>{if(!file)return; setName(file.name);setState("reading");try{const buf=await file.arrayBuffer();const head=new TextDecoder("latin1").decode(buf.slice(0,1024));if(!head.includes("%PDF"))throw new Error("El archivo seleccionado no es un PDF válido.");const items=await parseRosterPdf(buf);setCount(items.length);setState("done");onImported(file.name,items)}catch(e){setError(e instanceof Error?e.message:"No pudimos leer ese archivo.");setState("error")}};
...
export default function Home(){
  ...
  const agenda=shown; const next=shown.find(d=>d.type!=="off"&&d.type!=="leave")||shown[0];
  ...
  useEffect(()=>{if("serviceWorker" in navigator)navigator.serviceWorker.register("./sw.js").catch(()=>{});const saved=localStorage.getItem("rumbo-clock");if(saved){try{const p=JSON.parse(saved);setClock(p.clock);setOffset(p.offset)}catch{}}const roster=localStorage.getItem("rumbo-roster-data");if(roster){try{const p=JSON.parse(roster);if(Array.isArray(p.items)&&p.items.length){setSource(p.name||"Roster importado");setAllDuties(p.items);setVisibleMonth(p.month)}}catch{}}},[]);
  ...
  const imported=(name:string,items:Duty[])=>{setSource(name);setAllDuties(items);const counts=new Map<string,number>();items.forEach(d=>{const k=d.date.slice(0,7);counts.set(k,(counts.get(k)||0)+1)});const best=[...counts].sort((a,b)=>b[1]-a[1]||b[0].localeCompare(a[0]))[0][0];const month={y:Number(best.slice(0,4)),m:Number(best.slice(5,7))-1};setVisibleMonth(month);localStorage.setItem("rumbo-roster-data",JSON.stringify({name,items,month}));setFilter("all");setView("month")};
  return <main>
    ...
    <div className="shell">
      <section className="hero"><div><p className="eyebrow">{months[m].toUpperCase()} · {y}</p><h1>Hola, Leandro <span>👋</span></h1><p className="sub">Tu mes, listo para despegar.</p></div><div className="source"><span className="pulse"/><div><b>{source}</b><small>Disponible sin conexión</small></div><button aria-label="Opciones">•••</button></div></section>
      {next&&<section className="next-card" onClick={()=>setSelected(next)}><div className="date-box"><small>{weekdays[(new Date(next.date+"T12:00").getDay()+6)%7].toUpperCase()}</small><strong>{Number(next.date.slice(8))}</strong><span>{months[m].slice(0,3).toUpperCase()}</span></div><div className="next-main"><p>ACTIVIDAD DEL MES</p><h2><span className="plane">{icon[next.type]}</span> {next.title}</h2><div><span>◷ Presentación <b>{time(next)} {clock==="zulu"?"Z":""}</b></span>{next.reportPlace&&<span>⌖ {next.reportPlace}</span>}<span>{next.note}</span></div></div><button aria-label="Abrir detalle">›</button></section>}
